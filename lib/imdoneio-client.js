...
const log = require('debug')('imdone:imdoneio-client')
const Preferences = require('configstore')
...
module.exports = class ImdoneioClient extends Emitter {
  ...
  _auth (cb) {
    process.nextTick(() => {
      ...
      this.getAccount((err, user) => {
        log('err from getAccount on _auth', err)
        log('response from getAccount on _auth', user)
  ...
  saveCredentials () {
    log(`Saving credentials for ${this.email}`)
    prefs.set('key',authUtil.toBase64(`${this.email}:${this.password}`))
  }

  loadCredentials () {
    if (!prefs.has('key')) return
    let parts = authUtil.fromBase64(prefs.get('key')).split(':')
    this.email = parts[0]
    this.password = parts[1]
    log(`email: ${this.email} password: ${this.password}`)
  }

  removeCredentials () { prefs.delete('key') }
  ...
  createProject (repo, cb) {
    log(`Creating project for repo: ${repo.getPath()}`)
    log(`With config ${repo.config}`)
