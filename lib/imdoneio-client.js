...
module.exports = class ImdoneioClient extends Emitter {
  ...
  init (email, password) {
    if (email && password) {
      ...
      this.saveCredentials((err) => {
        if (err) return
        this.attemptAuth()
      })
    } else this.attemptAuth()
  }
  ...
  authFromStorage (cb) {
    ...
    return this.loadCredentials(err => {
      ...
      return this._auth((err, user) => {
        ...
        // TODO: if err.status == 404 we should show an error id:6 ok
        // return cb(err, user)
      })
    })
  }
  ...
  loadCredentials (cb) {
    return this.db().findOne({}, (err, doc) => {
      if (err) { return cb(err) }
      if (!doc) { return cb(AUTH_FAILED_ERR) }
