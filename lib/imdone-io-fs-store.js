module.exports = function (repo) {
  ...
  let CONFIG_DIR = require('imdone-core/lib/constants').CONFIG_DIR
  let ERRORS = require('imdone-core/lib/constants').ERRORS
  let client = require('./imdoneio-client').getInstance()
  let log = require('debug')('imdoneio-fs-store')
  let Task = require('imdone-core/lib/task')
  let fs = require('fs')
  ...
  repo.checkForIIOProject = (checkForIIOProject = function () {
    log('Checking for imdone.io project')

    if (repo.project) { return repo.emit('project.found', repo.project) }
    if (!client.isAuthenticated() || !repo.initialized) { return }
    if (!repo.getProjectId()) { return repo.emit('project.not-found') }
    return client.getProject(repo.getProjectId(), (err, project) => {
      // TODO: Do something with this error id:40 gh:116
      if (!project) {
        // repo.disableProject()
        if (!project) { return repo.emit('project.not-found') } // TODO: Handle the case where there is no project found. id:41 gh:116
      }
        // Check account for plan type
      if (err) { return (() => { throw err })() }
      repo.project = project
      repo.setProjectName(project.name)
      if (!repo.isImdoneIOProject()) { return }
  ...
  repo.syncTasks = syncTasks = function (tasks, cb) {
    ...
    return client.syncTasks(repo, tasks, function (err, ioTasks) {
      if (err) { return } // TODO: Do something with this error id:43 gh:116
  ...
  let syncFile = function (file, cb) {
    ...
    return client.syncTasks(repo, file.getTasks(), function (err, tasks) {
      if (err) { return } // TODO: Do something with this error id:48 gh:116
  ...
  repo.moveTasks = function (tasks, newList, newPos, cb) {
    let shouldSync = repo.isImdoneIOProject()
    if (cb == null) { cb = function () {} }
  ...
  repo.emitFileUpdate = function (file) {
    ...
    if (repo.shouldEmitFileUpdate(file)) {
      return syncFile(file, function (err, done) {
